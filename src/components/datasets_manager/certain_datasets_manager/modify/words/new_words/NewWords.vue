...
import NewWordsHeader from "@/components/datasets_manager/certain_datasets_manager/modify/words/new_words/header/NewWordsHeader.vue";
import AddWordForm from "@/components/datasets_manager/certain_datasets_manager/modify/words/new_words/AddWordForm.vue";
import NewWordsTable from "@/components/datasets_manager/certain_datasets_manager/modify/words/new_words/table/NewWordsTable.vue";
import DropFileHere from "@/components/datasets_manager/certain_datasets_manager/modify/words/new_words/DropFileHere.vue";

export default defineComponent({
    components: { AddWordForm, NewWordsTable, DropFileHere, NewWordsHeader },
    setup() {
        const { isDatasetJustCreated, useWordsManager, useImporting, isDeletingModalOpen } = useModifier;
        const { importOnDragAndDrop, dragging } = useImporting;
        const { currentWordsSection } = useWordsManager;

        const tabindex = computed<-1 | 1>(() => {
            return !isDeletingModalOpen.value && (currentWordsSection.value === "new" || isDatasetJustCreated.value) ? 1 : -1;
        });
